...
public class Lagern_agent extends Agent {
...
//--------------------------Initalisierung des Maschinenagentens------------------------------------------------------------------------------------------------	
	...
	public void setup() {
		...
		sd.setType("Lagern"); // Festlegung des individuellen Prozesses
	...
	private class Maschinensteuerung extends CyclicBehaviour {
		...
		public void action() {
			...
			if (Maschinendaten != null) {
				...
				else if (Maschinendaten.getConversationId().equals("R")) 
				
					try{
					
						float[][] state = new float[1][600]; // Zustandsvektor für das Deep q-Learning --> "MUST Provide as many biases as the Input"
						String Nachricht_Inhalt = Maschinendaten.getContent(); // Variablen für das Auslesen der Nachricht
						String Zwischenspeicher = "";
						char aChar;
						int Datenfeld = 0;
						...
						if (Anzahl_Aufrufe == Tensorflow_Maschine.get_warm_up_steps() && Initialization_agent.StartDqnMaschine == false) {
							System.out.println("DQN starten!");
							Start_dqn_maschine_100.Start_dqn_function();	
							Initialization_agent.StartDqnMaschine=true; // Start DQN eingefügt von Kemp
						}
