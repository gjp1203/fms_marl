package dqn_functions;
import org.tensorflow.*;
import org.tensorflow.Graph;

import java.util.Arrays;
import java.util.Random;
import java.util.concurrent.TimeUnit;
import java.lang.Math;
import debugging_functions.Start_dqn;
import supervisor_agents.Initialization_agent;

public class Tensorflow {
	
	private static int warm_up_steps = 3000;
	private final static int MAX_RETRY_COUNT=20;
	
	public static int action(float[][] state, int output_dim) { // graph g hier aufrufen lassen
		
		int retryCount = 1;	// Für Fehlerschleife, falls Graph nicht geladen wird.
		
		Graph g = null;
		double epsilon_Beginn = 1.2;
		//double epsilon_Ende = 0.01;
		double epsilon_Ende = 0.001;
		double epsilon_Zerfall = 0.9999;
		//double epsilon_Zerfall = 0.999;
		//int output_dim = 3;					// Abgeändert
		//int output_dim = 3;
		int a_t = 0;
		int i_DQN = 0;
		
		while (true) 
		{
			try {
					if (Transfer_replay_memory.getAnzahl_steps() == warm_up_steps && Initialization_agent.StartDqn == false) {
						Start_dqn.Start_dqn_function();
						Initialization_agent.StartDqn = true;
					}
				
					if (Transfer_replay_memory.getAnzahl_steps() >= Tensorflow.get_warm_up_steps()) {
						g = Load_graph.graph();
					}
		
					if (g == null|| Transfer_replay_memory.getAnzahl_steps() <= warm_up_steps) {
						Random action_t = new Random();
						a_t = action_t.nextInt(output_dim); // zufällige Aktion aus Aktionsvektor auswählen
						System.out.println("Random Action Verktor:" + a_t); 
						break;
					}
		
					else {
						double epsilon_aktuell = epsilon_Beginn * Math.pow(epsilon_Zerfall, (Transfer_replay_memory.getAnzahl_steps()-warm_up_steps)); 
							
													
							if (epsilon_aktuell < epsilon_Ende) {
								epsilon_aktuell = epsilon_Ende;
							}
		
						Random eps = new Random();
						Random action_t = new Random();
						
						double Whs = eps.nextFloat() * (epsilon_Beginn);
						System.out.println("Whs:" + Whs + "Epsilon_aktuell:" + epsilon_aktuell);
			
						if (Whs <= epsilon_aktuell) { // Exploration
							a_t = action_t.nextInt(output_dim); // zufällige Aktion aus Aktionsvektor auswählen
							break;
						}
						
						else if (Whs > epsilon_aktuell) {
				
							System.out.println("DQN Run Nr.:" + i_DQN);
							i_DQN = i_DQN++;
							System.out.println(Arrays.deepToString(state) + ":::::::::::::::::::::::");
								try (Session sess = new Session(g)) {
										System.out.println("Try Block gestartet");
										Tensor inputTensor = Tensor.create(state, Float.class); //Formatierung des Zustandsvektors als Tensor
										float[][] output = predict(sess, inputTensor);
										System.out.println(Arrays.deepToString(output) + "??????????????????????????????????????????????????????????????????????????????????????????????");
										
										inputTensor = null;
										
										// Interpretation des Aktionsvektors            
										double max_Output = 0;
										a_t = 0;
											for (int i = 0; i < output[0].length; i++) { // Aktion mit dem höchsten Ausgabewert im Aktionsvektor bestimmen
												System.out.println("Output[0][i]: " + output[0][i]);
												if (max_Output < output[0][i]) {
													max_Output = output[0][i];
													a_t = i;
												}
											}
							System.out.println(a_t + "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							sess.close();
							g.close();
							break;
						}
					}
				}
			}
			// Falls ein Fehler im Graphen vorhanden ist, kann dieser bis zu 5 mal geladen werden
			catch (Exception e){
				if (retryCount > MAX_RETRY_COUNT) {
					e.printStackTrace();
					throw new RuntimeException ("Tensorflow Error (MAX RETRY = 5)", e);
				}
					try {
						TimeUnit.SECONDS.sleep(1);
					}
					catch (InterruptedException ie) {
						Thread.currentThread().interrupt();
					}
				retryCount++;
				continue;
			}
		}
		g = null; 
		System.gc();
		return a_t; // Gewählte Aktion wird an den Auftragsagenten zurückgegeben
	}
	
	private static float[][] predict(Session sess, Tensor inputTensor) {
        Tensor result = sess.runner()
        		.feed("q_eval/input:0", inputTensor)
                .fetch("q_eval/output_layer/BiasAdd").run().get(0);
        float[][] outputBuffer = new float[1][Initialization_agent.Anzahl_Maschinen];
        result.copyTo(outputBuffer);
        result = null;
        return outputBuffer;
        
    }
	
	public static int get_warm_up_steps () {
    	return warm_up_steps;
    }
}