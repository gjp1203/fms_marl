...
class DeepQNetwork(object):
    def __init__(self, lr, n_actions, name, fc1_dim=8, fc2_dim=8, fc3_dim=8, input_dim=(8,), chkpt_dir='./dqn'):
        ...
        self.checkpoint_file = os.path.join(chkpt_dir,'deepqnet.ckpt_1_' + str(NumOfFiles))
    ...
    def save_to_pb(self): # Speichern des Funktionsapproximators für die Integration in die Auftragsagenten
        ...
        model_file = model_file = os.path.abspath(os.path.join(os.path.dirname( __file__ ), 'function_approximator/saved_network_1.pb'))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, batch_size, replace_target=10, input_dim=(11,),
        ...
        if os.path.split(os.getcwd())[1] == "multiagent":
            self.logname = 'losslog_1_' + str(NumberOfDocumentedFiles) +'.csv'
